...
public class FriendRequestsServlet extends HttpServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        cmdOut.println("friend_requests_for" + SEPARATOR + currentUsername);
        
        String resp;
        List<Profile> friendRequests = new ArrayList<Profile>();
        // retrieve response
        while (!(resp = in.readLine()).equals("END")) {
          if(!resp.equals("") && resp.indexOf("BAD ARGUMENTS") < 0) {
            friendRequests.add(Profile.fromCSV(resp));
          }
        }

        request.setAttribute("profiles", friendRequests);
