 ...
 * @author milandobrota
 */
public class FriendsFriendsServlet extends HttpServlet {
  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        cmdOut.println("friends_friends" + SEPARATOR + username);
        ...
        while (!(resp = in.readLine()).equals("END")) {
          if(!resp.equals("") && resp.indexOf("BAD ARGUMENTS") < 0) {
            friends.add(Profile.fromCSV(resp));
          }
        }

        request.setAttribute("profiles", friends);
